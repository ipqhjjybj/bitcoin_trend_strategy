...
class BitMEXWebsocket():
    ...
    def __init__(self , _apiKey , _secretKey):
    ...
    def connect(self, endpoint="", symbol_list=["XBTZ18","XBTUSD"], shouldAuth=True):
        ...
        for symbol in symbol_list:
            subscriptions += [sub + ':' + symbol for sub in ["instrument","quote", "trade", "orderBook10"]]
            if self.shouldAuth:
    ...
    def get_instrument(self, symbol):
        ...
        if 'instrument' not in self.data.keys():
            raise Exception(" instrument is not init success now {}".format(symbol))
    ...
    def get_orderbook(self, symbol):
        '''Return a depth object. Generated from orderbook10.'''
        if 'orderBook10' not in self.data.keys():
            raise Exception(" orderBook10 is not init success now {}".format(symbol))
        orderbooks = self.data["orderBook10"]
        match_orderbook_list = [i for i in orderbooks if i['symbol'] == symbol]
        orderbook_obj = match_orderbook_list[0]
        return orderbook_obj
    ...
    def get_use_orders(self, symbol, clOrdIDPrefix):
        orders = self.data['order']
        # Filter to only open orders (leavesQty > 0) and those that we actually placed
        return [o for o in orders if str(o['clOrdID']).startswith(clOrdIDPrefix) and o['leavesQty'] > 0 and o["symbol"] == symbol]
    ...
    def __connect(self, wsURL):
        ...
        if not conn_timeout or self._error:
            ...
            # 修改的地方
            # sleep(20)
            # self.logger.error("Now go to reconnect !")

            # # 连接
            # self.connect( self.use_endpoint , self.symbol_list )
    ...
    def __get_auth(self):
        ...
        return [
            "api-nonce: " + str(nonce),
            "api-signature: " + generate_signature(self.secretKey, 'GET', '/realtime', nonce, ''),
            "api-key:" + self.apiKey
        ]
    ...
    def __wait_for_account(self):
        ...
        self.logger.info("account wait function is ready!")
    ...
    def __wait_for_symbol(self, symbol):
        ...
        # 因为订阅了多个 symbol ,这里判断 symbol 是否全部加载完毕
        while not len(self.data["instrument"]) >= len(self.symbol_list):
            sleep(0.1)

        # 因为订阅了多个 symbol ,这里判断 symbol 是否全部加载完毕
        while not len(self.data["orderBook10"]) >= len(self.symbol_list):
            sleep(0.1)


        self.logger.info("Symbol wait function is ready!")
...
if __name__ == "__main__":
    API_KEY = "u5NV6qsjR1skajJDPu1-ULMR"
    API_SECRET = "VK_P7Kr5HuvgeMEclDpOZZQm7EhK7wFLnlMJHmKE8Za8oTt5"
    ...
    ws = BitMEXWebsocket( _apiKey = API_KEY, _secretKey = API_SECRET)
    ws.logger = logger
